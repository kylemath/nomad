function [best_labels, best_centers, best_intertia, best_group_sizes, i_iter] = equal_group_kmeans(X, n_groups, max_iter)
	...
	labels = int32(-1 * ones(n_samples, 1));
	...
	for i_iter = 1:max_iter
		...
		for point_id = isort(end:-1:1)
			...
			if ~((length(find(labels==cluster_id),1) >= n_samples/n_groups) & (point_cluster ~= cluster_id))
				labels(point_id) = cluster_id;
		...
		for group_id = 1:n_groups
			centers = [centers, mean(X(find(best_labels == group_id,1)),1)];
		end

		% update return values
		if best_intertia < 0 | inertia < best_intertia
			best_labels = labels;
			best_centers = centers;
			best_intertia = intertia;
		end
