...
% group_labels match these n rows of data_points
	...
	current_perm = mod(randperm(n_positions),group_size)+1;
	current_error = error(group_size, current_perm, distances);
	...
	while 1
		...
		for j = 1:n_positions
			for k = 1:j
				...
				foo = current_perm(j);
				current_perm(j) = current_perm(k);
				current_perm(k) = foo;

				error_post = error(group_size, current_perm, distances);
